...
Player::Player(XMFLOAT2 startpos,int pnum, bool isCPU) {
	...
	{
	case 1:
		m_effectType = SmashEffect1P;
			break;
	case 2:
		m_effectType = SmashEffect2P;
		break;
	case 3:
		m_effectType = SmashEffect3P;
		break;
	case 4:
		m_effectType = SmashEffect4P;
		break;
	default:
		break;
	}
...
void Player::Update() {
	...
	********************************************/
	...
	if (m_atkBuff&&!*m_buffEffectUse)
	{
		...
		EffectManager::CreateMoveEffect(BuffEffect, &m_pos, XMFLOAT2(80.0f, 100.0f), &m_rot, 0, XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f), &m_atkBuff, 1);
		m_buffEffectUse = &m_atkBuff;
	}
	if (m_defBuff && !*m_buffEffectUse)
	{
		EffectManager::CreateMoveEffect(BuffEffect, &m_pos, XMFLOAT2(80.0f, 100.0f), &m_rot, 0, XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f), &m_defBuff, 1);
		m_buffEffectUse = &m_defBuff;
	}
	if (m_moveDown && !*m_debuffEffectUse)
	{
		EffectManager::CreateMoveEffect(DebuffEffect, &m_pos, XMFLOAT2(80.0f, 100.0f), &m_rot, 0, XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f), &m_moveDown, 1);
		m_debuffEffectUse = &m_moveDown;
	}
	if (m_invert && !*m_debuffEffectUse)
	{
		EffectManager::CreateMoveEffect(DebuffEffect, &m_pos, XMFLOAT2(80.0f, 100.0f), &m_rot, 0, XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f), &m_invert, 1);
