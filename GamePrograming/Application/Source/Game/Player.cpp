...
void Player::Update() {
	...
	********************************************/
	...
	//ジャンプ（変更日：12/27 担当：弓田）
	//スペースキーかパッドの×ボタンが押されたか、かつジャンプした回数が2回未満だったら
...
{
	m_body->SetTransform(b2Vec2(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2), 0.0f);
}
...
{
	return m_pos.x <= -10.0f || m_pos.x >= SCREEN_WIDTH + 10.0f || m_pos.y <= -10.0f || m_pos.y >= SCREEN_HEIGHT + 10.0f;
}
