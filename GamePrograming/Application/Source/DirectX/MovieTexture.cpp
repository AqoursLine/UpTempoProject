...
bool MovieTexture::create(uint32_t new_xres, uint32_t new_yres, DXGI_FORMAT new_format, bool is_dynamic) {
	...
	HRESULT hr = D3D.GetDevice()->CreateTexture2D(&desc, nullptr, (ID3D11Texture2D**)&texture);
	if (FAILED(hr))
		return false;
	...
	hr = D3D.GetDevice()->CreateShaderResourceView(texture, &srv_desc, &shader_resource_view);
	if (FAILED(hr))
		return false;
...
void MovieTexture::activate(int slot) const {
	D3D.GetDeviceContext()->PSSetShaderResources(slot, 1, &shader_resource_view);
}
...
bool MovieTexture::updateFromIYUV(const uint8_t* data, size_t data_size) {
	...
	HRESULT hr = D3D.GetDeviceContext()->Map(texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &ms);
	if (FAILED(hr))
		return false;
